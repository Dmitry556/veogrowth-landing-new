...
Reference markdown for veogrowth — committed to git so it survives any local-disk loss, IDE reset, or AI-session compaction. **Reload these files into context to restore full project state.**
...
These docs are not served as routes by Vercel (no `index.html` inside `/docs`). They live in the repo for archival purposes only.
